...
function insertToBuffer(str, buffer, currentOffset) {
    return Buffer.concat([
        buffer.slice(0, currentOffset),
        Buffer.from(str, 'utf8'),
        buffer.slice(currentOffset + 1, buffer.length),
    ]);
}
/**
...
function parseType(parser, type) {
    switch (type) {
        ...
        default:
            getLogger().error('REDIS PARSER UNKNOWN TYPE', {
                buffer: parser.buffer.toString('utf8'),
                offset: parser.offset,
            });
            // In case we can single random character
            if (parser.buffer[this.offset + 1] === 13) {
                parser.buffer = insertToBuffer('CLUSTERDOWN', parser.buffer, parser.offset);
                getLogger().warn('REDIS PARSER INSERT TO BUFFER', {
                    buffer: parser.buffer.toString('utf8'),
                    offset: parser.offset,
                });
                return this.parseError(parser);
            }
            else {
